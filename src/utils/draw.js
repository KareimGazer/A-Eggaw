...
const setupHumidityGraph = (svg, data, width, height) => {
  ...
  gradient.append('stop').attr('offset', '30%').attr('stop-color', '#03fcc2').attr('stop-opacity', 0.7)

  gradient.append('stop').attr('offset', '45%').attr('stop-color', '#05e6c8').attr('stop-opacity', 0.7)

  gradient.append('stop').attr('offset', '80%').attr('stop-color', '#ffffff').attr('stop-opacity', 0.9)

  // Append the area under the minTemp (blue)
  svg
    .append('path')
    .datum(data)
    .attr('id', 'humidity-area')
