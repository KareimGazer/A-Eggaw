    ...
    if ('geolocation' in navigator && !pathname.includes('dashboard')) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          const current_city = worldCities.reduce((prev, curr) =>
            calcDistance(lat, lon, curr.latitude, curr.longitude) <
            calcDistance(lat, lon, prev.latitude, prev.longitude)
              ? curr
              : prev
          )
      ...
      <form onSubmit={onSubmit}>
        <fieldset className="join">
          ...
          <input
            ...
            className="btn btn-primary join-item rounded-full"
            type="submit"
            value={'Forecast'}
          />
        </fieldset>
